...
const VirtualBrowser = ({ defaultExpanded = true, onClose }) => {
  ...
  const [activeBackendTabId, setActiveBackendTabId] = useState(null);
  ...
  const initSession = useCallback(async () => {
    ...
    try {
      ...
      setActiveBackendTabId(session.tab_id || null);
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === 'tabs') {
          // Backend multi-tab list (we keep UI tabs as-is, but we can sync active backend tab)
          if (data.active_tab_id) {
            setActiveBackendTabId(data.active_tab_id);
          }
        }

        if (data.type === 'tab_activated') {
          if (data.tab_id) {
            setActiveBackendTabId(data.tab_id);
          }
        }

        if (data.type === 'status') {
          if (typeof data.can_go_back === 'boolean') setCanGoBack(data.can_go_back);
          if (typeof data.can_go_forward === 'boolean') setCanGoForward(data.can_go_forward);
        }
  ...
  // Update session status periodically (fallback if WS is down)
  useEffect(() => {
  ...
  const handleNewTab = useCallback(() => {
    ...
    // Create a real Playwright tab on the backend (best-effort, no UI breaking changes)
    if (sessionId) {
      browserApi.createTab(sessionId, null, true)
        .then((t) => {
          if (t?.tab_id) {
            setActiveBackendTabId(t.tab_id);
            setTabs(prev => prev.map(tab => tab.id === newTab.id ? { ...tab, backendTabId: t.tab_id } : tab));
            if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
              wsRef.current.send(JSON.stringify({ type: 'switch_tab', tab_id: t.tab_id }));
            }
          }
        })
        .catch(() => {});
  ...
  const handleTabClose = useCallback((tabId) => {
    setTabs(prev => {
      ...
      const backendTabId = closing?.backendTabId;
      if (sessionId && backendTabId) {
        browserApi.closeTab(sessionId, backendTabId).catch(() => {});
      }
  ...
  const handleTabActivate = useCallback((tabId) => {
    ...
    // If we have a backend tab id for this UI tab, activate it.
    const tab = tabs.find(t => t.id === tabId);
    const backendTabId = tab?.backendTabId;
    if (sessionId && backendTabId) {
      setActiveBackendTabId(backendTabId);
      browserApi.activateTab(sessionId, backendTabId).catch(() => {});
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        wsRef.current.send(JSON.stringify({ type: 'switch_tab', tab_id: backendTabId }));
      }
    }
  }, [tabs, sessionId]);
